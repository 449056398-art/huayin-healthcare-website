...
function EcosystemLoop() {
  return <div className="ecosystem-loop" aria-label="Self-reinforcing pathology intelligence loop">
    <svg className="ecosystem-lines" viewBox="0 0 720 430" aria-hidden="true"><defs><marker id="loop-arrow" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto"><path d="M0 0L8 4.5L0 9z" fill="#178bc8"/></marker></defs><path d="M360 94L158 335L562 335Z"/><path className="loop-segment segment-one" d="M360 108L176 328" markerEnd="url(#loop-arrow)"/><path className="loop-segment segment-two" d="M181 345L542 345" markerEnd="url(#loop-arrow)"/><path className="loop-segment segment-three" d="M553 325L370 108" markerEnd="url(#loop-arrow)"/><circle className="orbit orbit-one" cx="251" cy="220" r="5"/><circle className="orbit orbit-two" cx="430" cy="345" r="5"/><circle className="orbit orbit-three" cx="469" cy="215" r="5"/></svg>
...
function App() {
  ...
  return <div id="top">
    ...
    <main>
      ...
      <section className="china-coverage section-shell" id="coverage"><div className="coverage-copy"><p className="eyebrow blue">CHINA COVERAGE</p><h2>Local reach.<br/>Connected expertise.</h2><p>Huayin serves healthcare partners across 29 provincial-level regions in China. Our network includes 22 provincial-level service sites, with multiple sites operating in selected cities to meet local clinical and operational needs.</p><a className="text-button" href="#contact">Connect with Huayin <span>→</span></a></div><div className="coverage-panel" aria-label="China coverage and service site distribution visualization"><div className="coverage-map"><div className="map-stage"><img src={assetUrl('images/china-map-minimal.png')} alt="Minimal map of China including the South China Sea Islands" /><div className="marker-layer coverage-layer">{coverageMarkers.map(([name, left, top, , labelX, labelY]) => <span className="coverage-pin coverage" style={{ left: `${left}%`, top: `${top}%`, '--label-x': `${labelX}px`, '--label-y': `${labelY}px` }} data-label={name} title={name} aria-label={name} key={name}></span>)}</div><div className="marker-layer service-layer">{coverageMarkers.filter(([, , , type]) => type === 'service').map(([name, left, top, , labelX, labelY]) => <span className="coverage-pin service" style={{ left: `${left}%`, top: `${top}%`, '--label-x': `${labelX}px`, '--label-y': `${labelY}px` }} data-label={name} title={name} aria-label={name} key={name}></span>)}</div></div></div><div className="coverage-legend"><span className="coverage-view-label"><i className="coverage-dot"></i>Provincial-level coverage</span><span className="service-view-label"><i className="service-dot"></i>Service site distribution</span></div><div className="coverage-totals"><span className="coverage-view-label"><b>29</b> regions covered</span><span className="service-view-label"><b>22</b> service sites</span></div></div></section>
